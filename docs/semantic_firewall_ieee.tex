...
\filldraw[gray!40!black] (5.8,0.3) circle (2pt) node[left] {Safeguard};
% Pareto frontier
...
where $S \in [0,1]$ is normalized severity, $C$ is detector confidence, $P$ is pattern complexity, and $H$ is session-history evidence. The resulting value is mapped to risk levels (NONE, LOW, MEDIUM, HIGH, CRITICAL), and deployments select action thresholds according to their risk tolerance.
