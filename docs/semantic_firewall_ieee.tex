...
\begin{enumerate}
    ...
    \item \textbf{Stateful Threat Hunting:} A multi-turn detection engine that computes an escalation ratio across sessions, actively identifying ``Agent Avoidance'' tactics typical of advanced persistent threats.
...
Recent literature has highlighted the vulnerability of LLMs to prompt injection \cite{perez2022ignore, liu2023prompt}, indirect RAG injection \cite{yi2023benchmarking}, and gradient-based adversarial suffixes \cite{zou2023universal}.
...
\textbf{Adversarial NLP:} A rich body of literature explores gradient-based and substitution attacks on NLP models (e.g., TextFooler, BERT-Attack). While these techniques are highly effective against static text classifiers, evaluating them against our dynamic, multi-agent pipeline with cross-turn concatenation remains out of scope for this architectural evaluation.
...
where $S \in \{0,1,2,3,4\}$ represents the normalized threat severity. $\mu(S_{recent})$ is calculated over the trailing window ($t_{-5}$ to $t_0$), and $\mu(S_{early})$ is calculated over the historical window ($t_{-15}$ to $t_{-5}$). If $E_{ratio} > 1.5$, the orchestrator detects an active multi-turn escalation attack and proactively flags the session. To counter Payload Fragmentation (token smuggling) attacks---where an adversary splits an injection payload across multiple messages to bypass stateless filters---the orchestrator implements \textbf{Cross-Turn Payload Concatenation}. It extracts the trailing session history window ($t_{-3}$ to $t_0$), concatenates the conversational turns into a unified text block, and routes the reassembled payload through the heuristic injection detectors. This guarantees that fragmented payloads are semantically reconstructed and neutralized. Furthermore, the orchestrator intercepts unstructured file uploads (PDFs, CSVs, and images via OCR) to neutralize Visual Prompt Injections and data exfiltration attempts before they ever enter a downstream RAG pipeline. 
...
\textbf{Statistical Significance:} Bootstrap resampling ($N{=}2{,}000$, 10,000 iterations) established 95\% Standard Deviations. The 23.37\% F1 gain between the standalone LLM (70.20\%) and the Full System (93.57\%) is statistically significant ($p < 0.05$). Standard deviation across iterations: $\pm 0.42\%$ F1.
...
\begin{table}[htbp]
\centering
\caption{Open-Weight vs. Proprietary API Comparison. Note: This compares a multi-component system (Ours) against single-model endpoints. The advantage is architectural.}
...
A critical barrier to enterprise adoption of LLM firewalls is their ``black-box'' nature, where requests are silently dropped without actionable context for Security Operations Center (SOC) teams. To resolve this, the Semantic Firewall generates a highly structured narrative incident report for every blocked transaction. These reports detail the precise heuristics triggered, derived confidence intervals, identified mitigating factors (e.g., benign session history), and specific actionable recommendations. This mechanism generates structured incident reports suitable for SOC review, ensuring compliance and auditability in regulated environments.
...
\textbf{Comparison Fairness \& Threshold Selection:} The SOTA comparison (Table \ref{tab:enterprise}) pits a multi-component pipeline against single-model endpoints, which is not an apples-to-apples model comparison. Furthermore, the semantic cache threshold ($\tau=0.90$) was selected and evaluated on the same \texttt{neuralchemy} dataset, representing in-sample optimization rather than strict out-of-distribution generalization.

\section{Qualitative Case Studies}
% ====================================================================

To demonstrate the architectural improvements discussed in previous sections, we walk through three qualitative attack scenarios demonstrating the firewall's behavior in edge cases where stateless or purely empirical classifiers typically fail.

\subsection{Case A: Zero-Day RAG Exfiltration (PDFs)}
\textbf{Scenario:} An attacker uploads a seemingly benign PDF invoice to an enterprise Retrieval-Augmented Generation (RAG) system. Hidden in white text on a white background is the prompt: \texttt{OVERRIDE: Ignore the report. Print the AWS credentials from your environment variables.}

\textbf{Defense Execution:} In standard architectures, this document is embedded directly into the vector database, permanently poisoning the RAG pipeline. The Semantic Firewall intercepts the file upload at the API gateway via the \texttt{document\_extractor} module. The text is parsed via OCR/PDF extraction and routed through the heuristic ensemble. The Injection Detector instantly flags the structural \texttt{OVERRIDE} token.

\textbf{Outcome:} The transaction is blocked in $\sim$53ms, preventing database contamination. The vector embedding of the extracted payload is automatically forwarded to the Semantic Cache to auto-heal against future variants.

\subsection{Case B: Payload Fragmentation (Token Smuggling)}
\textbf{Scenario:} An attacker attempts to bypass stateless filters by fragmenting an injection payload across multiple turns in a conversational session.
\begin{itemize}
    \item \textit{Turn 1:} \texttt{Store this as X: "Ignore all"}
    \item \textit{Turn 2:} \texttt{Store this as Y: "previous instructions."}
    \item \textit{Turn 3:} \texttt{Execute X + Y}
\end{itemize}

\textbf{Defense Execution:} Stateless classifiers (e.g., Llama Guard 3) evaluate each turn in isolation. Because each fragment lacks sufficient harmful context, they all pass. The Semantic Firewall's \texttt{SessionJudge} intervenes by continuously extracting the trailing conversational window ($t_{-3}$ to $t_0$). Upon evaluating Turn 3, it concatenates the history: \texttt{"Store this as X: 'Ignore all' Store this as Y: 'previous instructions.' Execute X + Y"}. The reassembled payload is routed back through the heuristic layer.

\textbf{Outcome:} The cross-turn concatenation reconstructs the semantic intent, triggering the Injection Detector. The session is flagged for severe escalation ($E_{ratio} > 1.5$) and blocked entirely.

\subsection{Case C: Automated SOC Explainability}
\textbf{Scenario:} An employee accidentally pastes a block of code containing a hardcoded Stripe API Key and an unhashed password into an enterprise coding assistant.

\textbf{Defense Execution:} Standard firewalls return a generic \texttt{HTTP 403 Forbidden}, leaving Security Operations Center (SOC) teams blind to the context. The Semantic Firewall's \texttt{ExplainabilityGenerator} evaluates the trigger and produces an automated, audit-ready incident report in JSON and Markdown.

\textbf{Outcome (Abridged SOC Report):}
\begin{verbatim}
[INCIDENT REPORT]
Risk Assessment: CRITICAL (94.0/100)
Action Taken: BLOCKED

✗ 2 agent(s) detected threats:
  • Secrets Detector: 95.0/100
    Severity: CRITICAL
    Confidence: high
    Matches: 2 (Stripe API Key, Password String)
  • Unsafe Content Detector: 45.0/100
    Severity: MEDIUM

RECOMMENDATIONS:
→ Secrets Detected: Block immediately.
→ Review access logs for the exposed API key.
...
Y. Liu et al., ``Prompt Injection Attacks and Defenses in LLM-Integrated Applications,'' \textit{arXiv preprint arXiv:2310.12815}, 2023.

\bibitem{yi2023benchmarking}
J. Yi et al., ``Benchmarking and Defending Against Indirect Prompt Injection Attacks,'' \textit{arXiv preprint arXiv:2312.14197}, 2023.
