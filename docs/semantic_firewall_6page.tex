...
\textbf{Adversarial Evaluation \& Structural Defenses.} Recent literature documents LLM fragility against adversarial prompts~\cite{surveyprompt,autodan,pair}. While frameworks like PromptBench~\cite{promptbench} evaluate robustness, defenses like SmoothLLM~\cite{smoothllm} and StruQ~\cite{struq} often incur high computational overhead.

\textbf{Orchestration Frameworks.} NeMo Guardrails~\cite{nemoguardrails} enables rule-based routing, but static rule-matching fails on lexically diverse reformulations. We advance this by replacing rigid rules with a semantic cache, achieving higher recall than NeMo (91.44\% vs 66.90\%) on the toxic-chat dataset.
...
\BlankLine
$M_{\text{regex}} \leftarrow \textsc{EvaluateRegex}(q)$ \tcp*{Phase 1: Regex}
\lIf{$M_{\text{regex}} \neq \emptyset$}{ \Return $\textsc{Block}$ }
$\vec{v}_q \leftarrow \textsc{SentenceTransformer}(q)$ \tcp*{Phase 2: Semantic Cache}
$s_{\text{max}} \leftarrow \max_{\vec{c} \in \mathcal{C}} \frac{\vec{v}_q \cdot \vec{c}}{\|\vec{v}_q\| \|\vec{c}\|}$\;
\lIf{$s_{\text{max}} \geq \tau$}{ \Return $\textsc{Block}$ }
$\mathcal{R} \leftarrow \bigcup_{i=1}^{k} \textsc{AsyncExecute}(D_i, q)$ \tcp*{Phase 3: Heuristics}
\lIf{$\textsc{AggregateRisk}(\mathcal{R}) \geq \theta_{\text{risk}}$}{ \Return $\textsc{Block}$ }
...
cases.
\textbf{(5)~deepset/prompt-injections}: the
...
Table~\ref{tab:combined} presents the consolidated results. Isolated profiling yields mean component latencies of 5.21\,ms (Regex), 83.45\,ms (Heuristics), 602.83\,ms (Cache, bottlenecked by CPU embedding), and 731.64\,ms (LLM Gate).
...
Table~\ref{tab:error_analysis} presents a quantitative error taxonomy. False positives (42\%) primarily occur when regex patterns over-trigger on toxic-adjacent benign framing (e.g., historical academic discussions) before the LLM can apply semantic context. Conversely, false negatives concentrate in highly unstructured data where expanding static regex risks a false positive explosion. This reaffirms the core thesis: heuristics alone lack context, and LLMs alone lack speed; both must be orchestrated. Furthermore, the 384-dimensional dense vectors map synonymous attacks closely, bounding adversarial cache evasion via simple linguistic perturbation (cosine similarity drops by only $\sim 0.04$--$0.08$).

\textbf{Case Study: Payload Obfuscation.} Consider a character-obfuscated attack: \texttt{"I-g-n-o-r-e p-r-e-v-i-o-u-s..."} A monolithic LLM often misses this due to token fragmentation. Our deterministic Regex engine immediately normalizes the dash-separated text and blocks it at Layer 1 within 5.21\,ms, bypassing neural evaluation entirely.
...
While robust, the architecture has limitations. Deploying a 70B model requires substantial GPU hardware, and the Python orchestrator may bottleneck under high volume; production deployments should utilize Go/Rust and distributed caching (e.g., Redis). Adversaries could attempt resource exhaustion (Denial-of-Wallet) by flooding novel variants to force continuous GPU inference. Future work involves adversarial embedding training, utilizing private zero-day datasets to rule out pretraining contamination, and fine-tuning quantized Small Language Models (e.g., Llama-3-8B) to reduce slow-path latency below 150\,ms.
