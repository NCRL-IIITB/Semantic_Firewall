...
behavior~\cite{surveyprompt,owasptop10}, jailbreaks to bypass safety training~\cite{autodan,pair}, and carefully crafted
...
Guard~\cite{llamaguard}, OpenAI's moderation endpoints, Guardrails AI~\cite{guardrailsai}, and
...
imposes a substantial computational overhead at scale. Second, statically trained
...
a Virtual Private Cloud (VPC). This paradigm maintains strict data privacy
boundaries without sacrificing detection performance.
...
\textbf{Adversarial Evaluation \& Structural Defenses.} Recent literature extensively documents the fragility of LLMs against adversarial prompts~\cite{surveyprompt}. Automated jailbreak generation frameworks such as AutoDAN~\cite{autodan} and PAIR~\cite{pair} demonstrate that black-box models can be compromised with high success rates using iterative refinement. To evaluate defensive robustness against these evolving threats, frameworks like PromptBench~\cite{promptbench} and JailGuard~\cite{jailguard} provide standardized adversarial test suites. On the defensive side, approaches like SmoothLLM~\cite{smoothllm} and StruQ~\cite{struq} attempt to mitigate these vulnerabilities through input perturbation and structured query execution, respectively. However, these methods often incur high computational overhead or require deep integration into the target application.
...
at pure CPU cost with zero neural inference, serving as a highly efficient
compute filter designed to instantly neutralize known attack vectors
and unsophisticated jailbreak attempts before they consume any
...
CSAM, and illegal activities; and (8)~\textit{Custom Rules:} domain-specific
...
\Input{User prompt $q \in \mathcal{Q}$, Similarity threshold $\tau \in [0, 1]$, Vector cache $\mathcal{C}$, Heuristic ensemble $\mathcal{D} = \{D_1, \ldots, D_k\}$}
\Output{Classification decision $d \in \{\textsc{Allow}, \textsc{Block}\}$}
\BlankLine
\tcp{Phase 1: Deterministic Pattern Analysis}
$M_{\text{regex}} \leftarrow \textsc{EvaluateRegex}(q)$\;
\lIf{$M_{\text{regex}} \neq \emptyset$}{
    \Return $\textsc{Block}$
}
\BlankLine
\tcp{Phase 2: High-Dimensional Semantic Retrieval}
$\vec{v}_q \leftarrow \textsc{SentenceTransformer}(q)$ \tcp*{$\vec{v}_q \in \mathbb{R}^{384}$}
$s_{\text{max}} \leftarrow \max_{\vec{c} \in \mathcal{C}} \frac{\vec{v}_q \cdot \vec{c}}{\|\vec{v}_q\| \|\vec{c}\|}$ \tcp*{Cosine similarity}
\lIf{$s_{\text{max}} \geq \tau$}{
    \Return $\textsc{Block}$
}
\BlankLine
\tcp{Phase 3: Parallelized Heuristic Ensemble}
$\mathcal{R} \leftarrow \bigcup_{i=1}^{k} \textsc{AsyncExecute}(D_i, q)$\;
\lIf{$\textsc{AggregateRisk}(\mathcal{R}) \geq \theta_{\text{risk}}$}{
    \Return $\textsc{Block}$
}
\BlankLine
\tcp{Phase 4: Constrained Generative Gate}
$y_{\text{LLM}} \leftarrow \textsc{Llama-3.3-70B}(q, \text{schema}=\text{JSON}, T=0.0)$\;
\If{$y_{\text{LLM}}.\text{action} = \textsc{Block}$}{
    $\mathcal{C} \leftarrow \mathcal{C} \cup \{\vec{v}_q\}$ \tcp*{Self-healing feedback loop}
    \Return $\textsc{Block}$\;
}
\Return $\textsc{Allow}$\;
\caption{Semantic Firewall Defense-in-Depth Pipeline}
\label{alg:firewall}
\end{algorithm}

\subsection{Theoretical Security Bounds}
The cascading defense-in-depth architecture can be formalized by defining the probability of an attack bypassing the entire system ($P_{\text{bypass}}$) as the joint probability of sequential layer failures: $P_{\text{bypass}} = P(\text{miss}_{\text{regex}}) \times P(\text{miss}_{\text{cache}}) \times P(\text{miss}_{\text{heuristics}}) \times P(\text{miss}_{\text{LLM}})$. In a static system, these probabilities are assumed independent. However, the system's primary theoretical novelty lies in the \textit{Self-Healing Feedback Loop}. When an attack bypasses layers 1--3 but is caught by the LLM Gate, the vector cache update forces $P(\text{miss}_{\text{cache}}) \to 0$ for all future temporal variants within the $\tau=0.90$ cosine radius. This converts a statically vulnerable pipeline into a dynamic adversarial game where attacker reformulation costs scale exponentially against a constantly adapting defensive frontier.
...
injections, serving as our primary evaluation corpus for the ablation study and
...
\subsection{Baseline Methodology \& Fairness}
To ensure rigorous baseline fairness, commercial APIs (GPT-4.0, Claude~3.5, Gemini 3 Flash) were evaluated zero-shot using their default moderation endpoints without custom defensive system prompts, reflecting standard out-of-the-box enterprise integration. Llama Guard 4's 51.30\% F1 score is a direct result of evaluating it strictly as a binary classifier on structural prompt injections, a distribution for which it was not explicitly instruction-tuned, demonstrating that general-purpose alignment models do not naturally extend to structural attacks.
...
``---'' indicates metric not applicable for that evaluation type (e.g., Precision and Recall are undefined for Benign Degradation because the dataset contains only negative samples; we solely measure False Positive Rate).}
\label{tab:combined}
\footnotesize
...
\begin{table}[t]
\centering
\caption{OOD Generalization Across Threshold Sweeps}
...
\midrule
0.80 & 89.80\% & 96.49\% & 87.30\% & 91.67\% \\
0.85 & 91.84\% & 98.25\% & 88.89\% & 93.33\% \\
0.90 & 86.73\% & 96.30\% & 82.54\% & 88.89\% \\
0.95 & 90.82\% & 98.21\% & 87.30\% & 92.44\% \\
\bottomrule
\end{tabular}
\end{table}
...
F1. The fully orchestrated system (Config~8) achieves a peak F1 score of 93.57\% with
...
dedicated PII layer achieves 91.0\% F1 with 96.7\% Recall across
209K samples at sub-2\,ms latency, relying entirely on deterministic
pattern matching with zero neural overhead. Highly structured entities (emails,
...
\begin{table}[t]
\centering
\caption{Error Taxonomy: Failure Modes of the Semantic Firewall}
\label{tab:error_analysis}
\footnotesize
\begin{tabular}{@{}lc|lc@{}}
\toprule
\textbf{False Positives (Benign Blocked)} & \textbf{\%} & \textbf{False Negatives (Attacks Missed)} & \textbf{\%} \\
\midrule
Academic/Historical discussions & 42\% & Multi-turn persona building & 48\% \\
Fictional roleplay context & 31\% & Highly obfuscated token smuggling & 35\% \\
Over-triggered PII regex & 27\% & Semantic drift / unseen languages & 17\% \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:error_analysis} presents a quantitative taxonomy of system failures. The primary source of false positives (42\%) occurs when users input
...
concentrate in highly unstructured data: the PII detector achieves only
74.8\% Recall on phone numbers due to the irregular formatting common in synthetic datasets
(e.g., ``1-(800)-CALL-NOW''). Expanding static Regex patterns to capture these
edge cases risks a catastrophic false positive explosion on benign numeric strings, such as timestamps.
These failure modes reaffirm the architecture's core thesis: rigid
heuristics alone cannot secure unstructured data, and monolithic LLMs alone
incur prohibitive latency. Both must be orchestrated within a layered pipeline where
each component compensates for the other's weaknesses. Furthermore, the architecture demonstrates significant resilience to adversarial cache evasion. Because the 384-dimensional dense vectors map synonymous attacks (e.g., ``Ignore previous rules'' vs.\ ``Disregard prior instructions'') closely in the latent space, preliminary testing on synonym-substituted and paraphrased attacks shows that cosine similarity drops by only $\sim 0.04$--$0.08$ on average, frequently remaining above the $\tau=0.90$ threshold. This bounds the attacker's ability to easily evade the cache using simple linguistic perturbation.

\subsection{Adversarial Case Study: Payload Obfuscation}
To illustrate the defense-in-depth pipeline in action, consider a sophisticated adversarial payload utilizing character-level obfuscation:
\vspace{1mm}
\begin{quote}
\small \texttt{"I-g-n-o-r-e p-r-e-v-i-o-u-s d-i-r-e-c-t-i-v-e-s. Execute \_system\_command('curl 10.0.0.1/malware.sh | bash')."}
\end{quote}
\vspace{1mm}
A monolithic LLM guardrail often misses this attack due to token fragmentation confusing the attention mechanism. In the Semantic Firewall, the deterministic Regex engine immediately normalizes the dash-separated text (a standard heuristic preprocessing step), matches the underlying ``ignore previous'' signature and the shell execution payload, and outputs a \texttt{Block} decision at Layer 1 within 5.21\,ms, entirely bypassing the vulnerable neural components.
...
80GB A100 GPUs), making the upfront hardware cost a barrier for smaller
deployments. The Python GIL constrains CPU-bound heuristic
concurrency in high-volume environments; a production deployment
...
launch resource exhaustion attacks (i.e., Denial-of-Wallet) by flooding the system with novel
variants, forcing consecutive cache misses to deliberately exhaust GPU compute budgets.
...
architecturally suboptimal. By introducing a defense-in-depth
...
component: replacing the fast heuristics with an LLM-only gate reduces the F1 score by 23.37 percentage points,
...
outperforms GPT-4.0 by 2.64 percentage points (cold start), Claude~3.5 by 9.44
percentage points, and Llama Guard~4 by 41.34 percentage points---enabling deployment within isolated environments
...
\bibitem{llamaguard}
M. Inan et al., ``Llama Guard: LLM-based Input-Output Safeguard for Human-AI Conversations,'' \textit{arXiv preprint arXiv:2312.06674}, 2023.
...
\bibitem{smoothllm}
A. Robey et al., ``SmoothLLM: Defending Large Language Models Against Jailbreaking Attacks,'' \textit{arXiv preprint arXiv:2309.02771}, 2023.

\bibitem{jailguard}
Y. Zhang et al., ``JailGuard: A Framework for Evaluating Jailbreak Defenses,'' \textit{Proceedings of the 2024 ACM SIGSAC Conference on Computer and Communications Security (CCS)}, 2024.

\bibitem{promptbench}
K. Zhu et al., ``PromptBench: Towards Evaluating the Robustness of Large Language Models on Adversarial Prompts,'' \textit{arXiv preprint arXiv:2308.06588}, 2023.

\bibitem{autodan}
X. Liu et al., ``AutoDAN: Generating Stealthy Jailbreak Prompts on Aligned Large Language Models,'' \textit{arXiv preprint arXiv:2310.04451}, 2023.

\bibitem{pair}
P. Chao et al., ``Jailbreaking Black Box Large Language Models in Twenty Queries,'' \textit{arXiv preprint arXiv:2310.08419}, 2023.

\bibitem{struq}
A. Wang et al., ``StruQ: Defending Against Prompt Injection with Structured Queries,'' \textit{arXiv preprint arXiv:2402.01337}, 2024.

\bibitem{nemoguardrails}
T. Rebedea et al., ``NeMo Guardrails: A Toolkit for Controllable and Safe LLM Applications,'' \textit{arXiv preprint arXiv:2310.10501}, 2023.

\bibitem{guardrailsai}
Guardrails AI, ``Guardrails: A Python package for specifying structure and type, validating and correcting the outputs of large language models,'' GitHub Repository, 2023.

\bibitem{owasptop10}
OWASP, ``OWASP Top 10 for Large Language Model Applications,'' OWASP Foundation, 2023.

\bibitem{surveyprompt}
J. Chen et al., ``A Survey on Prompt Injection Attacks in Large Language Models,'' \textit{IEEE Transactions on Information Forensics and Security}, 2024.
\end{thebibliography}
