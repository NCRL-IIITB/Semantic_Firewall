...
\end{tabular}
\vspace{1mm} \\
\raggedright \textit{*Note: Cache latency reflects CPU embedding generation via SentenceTransformers. Migrating this layer to a T4 GPU reduces embedding latency to $\sim$20ms, which would further drop steady-state $L_{eff}$ significantly.}
\end{table}
...
structural prompt injections (Llama Guard~4 was evaluated using the exact prompt formatting and taxonomy prescribed in Meta's official documentation to ensure a fair baseline). Statistical significance was verified
...
(Note: Cache evaluation guarantees zero data leakage as the vector database was warmed strictly on a disjoint 5,000-sample training split).
