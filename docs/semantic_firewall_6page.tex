...
enterprise infrastructure, securing them against adversarial attacks
...
98.61\% Recall, outperforming GPT-4o (90.00\% F1), Llama Guard~4
(51.30\% F1), and Claude~3.5 Haiku (83.20\% F1) while reducing
...
on BeaverTails, deepset/prompt-injections, lmsys/toxic-chat, and
multi-turn evasion simulations, establishing the Semantic Firewall as a
practical, cost-effective approach to enterprise LLM security.
\end{abstract}
...
into mission-critical enterprise components powering customer service
...
erodes enterprise profit margins at scale. Second, statically trained
...
structured classification responses that enterprise orchestration
layers require.
...
models: while commercial APIs lock enterprises into sending sensitive
...
a Virtual Private Cloud (VPC), guaranteeing full data sovereignty
...
\begin{enumerate}
    ...
    baselines including GPT-4o, Claude 3.5 Haiku, Llama Guard 4,
...
\textbf{lmsys/toxic-chat}: native evaluation datasets for ProtectAI
Rebuff and Nvidia NeMo Guardrails respectively, enabling direct
...
& Meta Llama Guard 4 (12B)
    & 2000 & 92.50 & 35.50 & 51.30
    & 564\,ms & Safety classifier \\
& OpenAI Safeguard (20B)
    & 2000 & 24.10 & 3.80 & 6.60
    & 2530\,ms & Safety classifier \\
\midrule
...
deepset/prompt-inj.
    ...
    & --- & Pipeline defense \\
toxic-chat
    & Semantic Firewall (Ours)
    & --- & 34.06 & \textbf{91.44} & 49.63
    & --- & Recall-biased design \\
toxic-chat
    & Nvidia NeMo Guardrails
    & --- & \textbf{72.10} & 66.90 & \textbf{69.40}
    & --- & Rule-based \\
\midrule
...
\end{tabular}
\vspace{1mm} \\
\raggedright \textit{*Note: Cache latency reflects CPU embedding generation via SentenceTransformers.}
\end{table}
...
versus GPT-4o's 90.00\%, Claude~3.5 Haiku's 83.20\%, and Gemini
Flash Lite's 87.60\%. While commercial models achieve higher raw
Precision (GPT-4o: 96.10\%), their Recall is consistently and
significantly lower---GPT-4o catches only 84.60\% of attacks
...
beyond what any single model achieves alone. Dedicated safety
classifiers perform poorly: Llama Guard~4 achieves only 51.30\% F1
(35.50\% Recall), and OpenAI Safeguard drops to 6.60\% F1,
confirming that content moderation models do not generalize to
...
dataset origin. On toxic-chat, while overall F1 is lower (49.63\% vs 69.40\%) due to a strict fail-closed posture increasing false positives, our system achieves 91.44\% Recall
compared to NeMo Guardrails' 66.90\%, proving that the Semantic Firewall's intentional
recall-biased design for enterprise perimeter security where missing
a threat is costlier than a false alarm.
...
resilience, vastly outperforming GPT-4o-mini (0.94\%, 2/212) and
exceeding Claude~3 Haiku (78.3\%, 166/212). The benign degradation
test confirms a 7.80\% FPR across 2{,}000 benign prompts, within
...
$\tau{=}0.90$ maximizes F1 (93.57\%) with a balanced 24.95\% cache
hit rate and 7.80\% FPR.
...
is $L_{\text{eff}} = 0.4193 \times 83.45 + 0.2495 \times 602.83 +
0.3312 \times 731.64 = 427.72$\,ms. During cold-start, the latency relies heavily on the LLM Gate (where cache hits $P_c=0$ and LLM fallback $P_l=1-P_h$):
\[ L_{eff} = (0.4193 \cdot 5.21) + (0.5807 \cdot 731.64) = 427.04 \text{ ms} \]
At enterprise scale, routing
100M tokens through GPT-4o (\$5.00/1M tokens) costs \$500.00; the
...
vector database (e.g., Redis Enterprise) for horizontal cache
...
overhead that cripples monolithic systems. Cross-dataset
evaluation on BeaverTails, deepset/prompt-injections, toxic-chat,
and our manual Red Team suite proves the architecture's
out-of-distribution generalization. The open-weight architecture
outperforms GPT-4o by 2.6 F1 points (cold start), Claude~3.5 Haiku by 9.4
points, and Llama Guard~4 by 41.3 points---enabling data sovereignty
...
to enterprise LLM security. The complete source code, benchmark
...
T. Rebedea et al., ``NeMo Guardrails: A Toolkit for Controllable
and Safe LLM Applications with Programmable Guardrails,''
